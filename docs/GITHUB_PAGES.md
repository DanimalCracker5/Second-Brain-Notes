...
as the Pages source; it contains `index.html`, which opens the app in `app/`.
...
2. Upload the contents of this project, keeping the folders intact:
   `index.html`, `app/`, `backups/`, `firebase.json`, `firestore.rules`, and
...
The `docs/` folder is documentation, not the Pages publishing source. If you
select `/docs` in GitHub Pages, GitHub will publish this guide instead of the
app because the app files are in `app/`. Keep the Pages source set to root.

The Firebase web config in `app/astral.config.js` is expected to be public for
